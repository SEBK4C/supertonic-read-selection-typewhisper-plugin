...
final class SupertonicPluginTests: XCTestCase {
    ...
    func testSelectVoiceSpeedPlaybackDSPQualityAndInferenceBackendPersistChoices() throws {
        ...
        plugin.setTimePitchOverlap(16)
        plugin.setLowPassEnabled(true)
        plugin.setLowPassCutoff(7_500)
        plugin.setResonanceEQEnabled(true)
        plugin.setResonanceEQFrequency(850)
        plugin.setResonanceEQGain(-4.5)
        plugin.setResonanceEQBandwidth(1.2)
        plugin.setClarityEnabled(true)
        plugin.setClarityFrequency(4_000)
        plugin.setClarityGain(2.5)
        plugin.setSibilanceTamerEnabled(true)
        plugin.setSibilanceFrequency(7_000)
        plugin.setSibilanceGain(-5.0)
        plugin.setSibilanceBandwidth(0.8)
        plugin.setAutomaticExpressionTagsEnabled(true)
        plugin.setBreathBetweenSentencesEnabled(true)
        plugin.setLaughShorthandEnabled(true)
        plugin.setSighForEllipsesEnabled(true)
        ...
        XCTAssertEqual(plugin.selectedTimePitchOverlap, 16, accuracy: 0.001)
        XCTAssertEqual(plugin.selectedLowPassEnabled, true)
        XCTAssertEqual(plugin.selectedLowPassCutoff, 7_500, accuracy: 0.001)
        XCTAssertEqual(plugin.selectedResonanceEQEnabled, true)
        XCTAssertEqual(plugin.selectedResonanceEQFrequency, 850, accuracy: 0.001)
        XCTAssertEqual(plugin.selectedResonanceEQGain, -4.5, accuracy: 0.001)
        XCTAssertEqual(plugin.selectedResonanceEQBandwidth, 1.2, accuracy: 0.001)
        XCTAssertEqual(plugin.selectedClarityEnabled, true)
        XCTAssertEqual(plugin.selectedClarityFrequency, 4_000, accuracy: 0.001)
        XCTAssertEqual(plugin.selectedClarityGain, 2.5, accuracy: 0.001)
        XCTAssertEqual(plugin.selectedSibilanceTamerEnabled, true)
        XCTAssertEqual(plugin.selectedSibilanceFrequency, 7_000, accuracy: 0.001)
        XCTAssertEqual(plugin.selectedSibilanceGain, -5.0, accuracy: 0.001)
        XCTAssertEqual(plugin.selectedSibilanceBandwidth, 0.8, accuracy: 0.001)
        XCTAssertEqual(plugin.selectedAutomaticExpressionTagsEnabled, true)
        XCTAssertEqual(plugin.selectedBreathBetweenSentencesEnabled, true)
        XCTAssertEqual(plugin.selectedLaughShorthandEnabled, true)
        XCTAssertEqual(plugin.selectedSighForEllipsesEnabled, true)
        ...
        XCTAssertEqual(host.userDefault(forKey: "timePitchOverlap") as? Double, 16)
        ...
        XCTAssertEqual(host.userDefault(forKey: "resonanceEQEnabled") as? Bool, true)
        XCTAssertEqual(host.userDefault(forKey: "resonanceEQFrequency") as? Double, 850)
        XCTAssertEqual(host.userDefault(forKey: "resonanceEQGain") as? Double, -4.5)
        XCTAssertEqual(host.userDefault(forKey: "resonanceEQBandwidth") as? Double, 1.2)
        XCTAssertEqual(host.userDefault(forKey: "clarityEnabled") as? Bool, true)
        XCTAssertEqual(host.userDefault(forKey: "clarityFrequency") as? Double, 4_000)
        XCTAssertEqual(host.userDefault(forKey: "clarityGain") as? Double, 2.5)
        XCTAssertEqual(host.userDefault(forKey: "sibilanceTamerEnabled") as? Bool, true)
        XCTAssertEqual(host.userDefault(forKey: "sibilanceFrequency") as? Double, 7_000)
        XCTAssertEqual(host.userDefault(forKey: "sibilanceGain") as? Double, -5.0)
        XCTAssertEqual(host.userDefault(forKey: "sibilanceBandwidth") as? Double, 0.8)
        XCTAssertEqual(host.userDefault(forKey: "automaticExpressionTagsEnabled") as? Bool, true)
        XCTAssertEqual(host.userDefault(forKey: "breathBetweenSentencesEnabled") as? Bool, true)
        XCTAssertEqual(host.userDefault(forKey: "laughShorthandEnabled") as? Bool, true)
        XCTAssertEqual(host.userDefault(forKey: "sighForEllipsesEnabled") as? Bool, true)
    ...
    func testPlaybackAndInferenceDefaults() throws {
        ...
        XCTAssertEqual(plugin.selectedTimePitchOverlap, 8.0)
        XCTAssertEqual(plugin.selectedLowPassEnabled, false)
        XCTAssertEqual(plugin.selectedLowPassCutoff, 9_000)
        XCTAssertEqual(plugin.selectedResonanceEQEnabled, false)
        XCTAssertEqual(plugin.selectedResonanceEQFrequency, 900)
        XCTAssertEqual(plugin.selectedResonanceEQGain, -3.0)
        XCTAssertEqual(plugin.selectedResonanceEQBandwidth, 1.0)
        XCTAssertEqual(plugin.selectedClarityEnabled, false)
        XCTAssertEqual(plugin.selectedClarityFrequency, 3_500)
        XCTAssertEqual(plugin.selectedClarityGain, 2.0)
        XCTAssertEqual(plugin.selectedSibilanceTamerEnabled, false)
        XCTAssertEqual(plugin.selectedSibilanceFrequency, 6_500)
        XCTAssertEqual(plugin.selectedSibilanceGain, -3.0)
        XCTAssertEqual(plugin.selectedSibilanceBandwidth, 1.0)
        XCTAssertEqual(plugin.selectedAutomaticExpressionTagsEnabled, false)
        XCTAssertEqual(plugin.selectedBreathBetweenSentencesEnabled, false)
        XCTAssertEqual(plugin.selectedLaughShorthandEnabled, false)
        XCTAssertEqual(plugin.selectedSighForEllipsesEnabled, false)
    ...
    func testExpressionTaggerAddsOnlyEnabledConfirmedTags() {
        let settings = SupertonicExpressionSettings(
            automaticTagsEnabled: true,
            breathBetweenSentencesEnabled: true,
            laughShorthandEnabled: true,
            sighForEllipsesEnabled: true
        )

        let output = SupertonicExpressionTagger.apply(to: "Hello. (lol) Wait... Okay?", settings: settings)

        XCTAssertEqual(output, "Hello. <breath> <laugh> Wait <sigh> Okay?")
    }
    ...
    func testReadSelectionActionSpeaksInputText() async throws {
        ...
        plugin.setTimePitchOverlap(12)
        plugin.setLowPassEnabled(true)
        plugin.setLowPassCutoff(8_000)
        plugin.setResonanceEQEnabled(true)
        plugin.setResonanceEQFrequency(750)
        plugin.setResonanceEQGain(-6)
        plugin.setResonanceEQBandwidth(1.5)
        plugin.setClarityEnabled(true)
        plugin.setClarityFrequency(4_500)
        plugin.setClarityGain(3)
        plugin.setSibilanceTamerEnabled(true)
        plugin.setSibilanceFrequency(7_500)
        plugin.setSibilanceGain(-4.5)
        plugin.setSibilanceBandwidth(0.7)
        plugin.setAutomaticExpressionTagsEnabled(true)
        plugin.setBreathBetweenSentencesEnabled(true)
        plugin.setLaughShorthandEnabled(true)
        ...
        let result = try await plugin.execute(
            input: "  Selected text. (lol) More text.  ",
        ...
        XCTAssertEqual(synthesizer.lastRequest?.text, "Selected text. <breath> <laugh> More text.")
        ...
        XCTAssertEqual(sessionFactory.latestSession?.playbackSettings.timePitchOverlap ?? 0, 12, accuracy: 0.001)
        ...
        XCTAssertEqual(sessionFactory.latestSession?.playbackSettings.resonanceEQEnabled, true)
        XCTAssertEqual(sessionFactory.latestSession?.playbackSettings.resonanceEQFrequency ?? 0, 750, accuracy: 0.001)
        XCTAssertEqual(sessionFactory.latestSession?.playbackSettings.resonanceEQGain ?? 0, -6, accuracy: 0.001)
        XCTAssertEqual(sessionFactory.latestSession?.playbackSettings.resonanceEQBandwidth ?? 0, 1.5, accuracy: 0.001)
        XCTAssertEqual(sessionFactory.latestSession?.playbackSettings.clarityEnabled, true)
        XCTAssertEqual(sessionFactory.latestSession?.playbackSettings.clarityFrequency ?? 0, 4_500, accuracy: 0.001)
        XCTAssertEqual(sessionFactory.latestSession?.playbackSettings.clarityGain ?? 0, 3, accuracy: 0.001)
        XCTAssertEqual(sessionFactory.latestSession?.playbackSettings.sibilanceTamerEnabled, true)
        XCTAssertEqual(sessionFactory.latestSession?.playbackSettings.sibilanceFrequency ?? 0, 7_500, accuracy: 0.001)
        XCTAssertEqual(sessionFactory.latestSession?.playbackSettings.sibilanceGain ?? 0, -4.5, accuracy: 0.001)
        XCTAssertEqual(sessionFactory.latestSession?.playbackSettings.sibilanceBandwidth ?? 0, 0.7, accuracy: 0.001)
    }
